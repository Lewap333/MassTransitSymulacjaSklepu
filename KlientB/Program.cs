    ...
    internal class Program
    {
        static void Main()
        {
            var bus = Bus.Factory.CreateUsingRabbitMq(cfg =>
            {
                ...
                cfg.ReceiveEndpoint(host, "kolejkaOdbioruB", ep =>
                {
            ...
            bus.Start();
            ConsoleCol.WriteLine("[KlientB] Uruchomiono. Wpisz liczbę sztuk lub S/T/Q.", ConsoleColor.White);
                ...
                switch (input.ToUpperInvariant())
                {
                    ...
                    case "S":
                        if (HandlerClient.BiezaceId.HasValue)
                        {
                            ...
                            ConsoleCol.WriteLine("[KlientB] Wysłano POTWIERDZENIE", ConsoleColor.Green);
                    ...
                    case "T":
                        if (HandlerClient.BiezaceId.HasValue)
                        {
                            ...
                            ConsoleCol.WriteLine("[KlientB] Wysłano BRAK potwierdzenia", ConsoleColor.Red);
                            HandlerClient.BiezaceId = null;
                        }
                        else
                    ...
                    default:
                        if (int.TryParse(input, out int ilosc) && ilosc > 0)
                        {
                            ...
                            ConsoleCol.WriteLine($"[KlientB] Wysłano zamówienie {ilosc} szt. (id={id})", ConsoleColor.Cyan);
                        }
                        else
